    ...
    if (Array.isArray(data)) {
        ...
        for (const g of data) {
            if (typeof g === "string") {
                ...
                const catName =
                    g.name || g.category || g.title || g.label || "";
...
export const LatihanPage: React.FC = () => {
    ...
    const [speechResults, setSpeechResults] = useState<
        Record<
            number,
            { transcript: string; score: number; diffParts: DiffPart[] }
        >
    >({});
    ...
    const handleSaveRefleksi = async (masterList: any[]) => {
        try {
            ...
            const answersPayload = masterList.map((item: any, idx: number) => {
                ...
                const userAns =
                    refleksiAnswers[mrId || idx] ??
                    refleksiAnswers[qText] ??
                    "";
                return {
    ...
    const [gradeNotification, setGradeNotification] = useState<{
        title: string;
        grade: string;
    ...
    const triggerGradeNotification = (
        title: string,
        grade: string,
        feedback: string | null,
    ) => {
                    ...
                    try {
                        ...
                        if (currentAssigns.length > 0) {
                            filteredAssigns.forEach((newAssign: any) => {
                                const oldAssign = currentAssigns.find(
                                    (a: any) => a.id === newAssign.id,
                                );
                                if (!oldAssign) return;

                                const newSub =
                                    newAssign.submissions &&
                                    newAssign.submissions[0];
                                const oldSub =
                                    oldAssign.submissions &&
                                    oldAssign.submissions[0];
                        ...
                        if (filteredAssigns.length > 0) {
                            const commentPromises = filteredAssigns.map(
                                (assign: any) =>
                                    api.lms.comments.list({
                                        kanjiId: kanjiData.id,
                            ...
                            console.log(
                                "[LMS Student Poll] Loaded comments:",
                                allComms,
                            );
    ...
    const getCorrectAnswerText = (currentQ: QuizQuestion) => {
        ...
        if (rawCorr === undefined || rawCorr === null || rawCorr === "")
            return "";
        ...
        const fetchKanji = async () => {
            try {
                ...
                if (data.xpEarned > 0) {
                    triggerXpReward(
                        data.xpEarned,
                        "Membuka modul pembelajaran baru",
                    );
                    playPageLoadSound();
                }
                ...
                if (cached) {
                    ...
                    const readList = data.examples.filter(
                        (ex: any) => ex.isReading,
                    );
                    const totalCount =
                        readList.length > 0
    ...
    const handleVerify = async () => {
        ...
        if (!validation) {
            setVerificationInfo({
                ...
                incorrectStrokes: [],
                feedbackMessage:
                    "Gagal menganalisa goresan. Silakan coba lagi.",
                infoText: "",
        ...
        if (stars === 5) {
            feedbackMessage =
                "Sempurna! Urutan dan arah goresan Anda sangat akurat.";
        } else if (stars === 4) {
            feedbackMessage =
            ...
            feedbackMessage =
                "Coba lagi! Perhatikan urutan goresan yang salah.";
        } else {
            feedbackMessage =
    ...
    const handleToggleReading = async (idx: number, isCompleted: boolean) => {
        ...
        try {
            const readList = kanjiData.examples.filter(
                (ex: any) => ex.isReading,
            );
            const totalCount =
                readList.length > 0
                    ? readList.length
                    : kanjiData.examples.length;
            const checkedCount =
    ...
    const submitQuizScore = async (score: number) => {
        ...
        try {
            ...
            if (response.xpEarned > 0) {
                triggerXpReward(
                    response.xpEarned,
                    `Menyelesaikan kuis evaluasi kanji`,
                );
            } else if (score >= 75) {
                confetti();
            }
                ...
                {activeTab === "detail" && (
                    <div className="flex flex-col gap-4">
                                ...
                                <table className="w-full text-left text-xs border-collapse">
                                    <thead>
                                        <tr className="bg-slate-100/80 text-slate-700 font-bold border-b border-slate-200/70">
                                            <th className="p-3.5 w-1/3 border-r border-slate-200/70">
                                                Informasi
                                            </th>
                                    ...
                                    <tbody className="divide-y divide-slate-100 text-slate-800">
                                        <tr>
                                            <td className="p-3.5 font-bold bg-slate-50/50 border-r border-slate-200/50">
                                                Kanji
                                            </td>
                                            <td className="p-3.5 font-black text-xl text-[#8f0020] font-mono">
                                                {kanjiData?.character ||
                                                    kanjiData?.kanji}
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            ...
                                            <td className="p-3.5 font-extrabold text-[#8f0020] font-mono">
                                                {kanjiData?.onyomi || "-"}
                                            </td>
                                        </tr>
                                        <tr>
                        ...
                        <div id="semantic-graph-section" className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs animate-zoom-in overflow-hidden flex flex-col gap-2">
                                ...
                                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs select-none">
                                    <div className="bg-white border border-slate-200 shadow-2xl rounded-2xl max-w-[480px] w-full p-6 flex flex-col gap-4">
                                        ...
                                        <div className="text-sm text-slate-600 space-y-4 text-left">
                                            <p>
                                                Peta visual interaktif yang
                                                menggambarkan relasi etimologis
                                                dan pembentukan kata gabungan
                                                (Jukugo) dari Kanji yang sedang
                                                dipelajari.
                                            </p>
                                            <div>
                            ...
                            <div className="relative h-200 w-full flex items-center justify-center bg-slate-50/50 border border-slate-100 rounded-2xl overflow-hidden">
                                {graph &&
                                    graph.nodes &&
                                    graph.nodes.length > 0 && (
                                        <KanjiAtlasFlow
                                            ...
                                            activeJukugoWord={activeJukugoWord}
                        ...
                        <div id="stroke-order-section" className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs flex flex-col">
                            ...
                            <div className="flex flex-col gap-4">   
                                ...
                                <div className="flex flex flex-col sm:flex-row gap-4">
                                    ...
                                    {/* Canvas Pad */}
                                    <div className="flex-1">
                                        <div className="w-full h-full flex flex-col sm:flex-row items-center justify-center gap-6 w-full">
                            ...
                            return (
                                <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs select-text animate-zoom-in space-y-4">
                                    ...
                                    <p className="text-xs text-slate-500 font-medium">
                                        Tuliskan jawaban refleksi Anda untuk
                                        kanji{" "}
                                        <strong className="text-[#8f0020]">
                                            {kanjiData?.character}
                                        </strong>{" "}
                                        berikut:
                                    </p>
                    ...
                    <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs space-y-6 text-left max-w-4xl mx-auto w-full select-text">
                        ...
                        <div className="space-y-6">
                            ...
                            {examples.length === 0 && (
                                <p className="text-sm text-slate-400 italic">
                                    Belum ada kalimat latihan membaca yang
                                    dimuat untuk kanji ini.
                                </p>
                            )}
                        </div>
                        ...
                        <div className="border-t border-slate-100 pt-6 flex justify-between items-center">
                            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                                PROGRES:{" "}
                                {
                                    Object.values(readSentences).filter(Boolean)
                                        .length
                                }{" "}
                                / {examples.length} SELESAI
                            </div>
                    ...
                    <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs max-w-2xl mx-auto w-full select-text text-left min-h-[380px] flex flex-col gap-6">
                                    ...
                                    {quizQuestions[currentQuestionIdx].type ===
                                        ...
                                        <div className="space-y-3 mt-4">
                                            {(
                                                ...
                                                const isCorrect =
                                                    matchingCorrect[pair.left];
                                                const isWrong =
                                                    matchingWrong[pair.left];
                                                return (
                                                    <div
                                                        key={pIdx}
                                    ...
                                    {quizQuestions[currentQuestionIdx].type ===
                                        ...
                                        <div className="space-y-4 mt-4 animate-scale-up">
                                            <p className="text-xs font-extrabold text-slate-500 italic select-none">
                                                Pilihlah kelompok/kategori yang
                        ...
                        {quizQuestions.length === 0 && (
                            <div className="flex-1 flex flex-col items-center justify-center text-center">
                                <p className="text-sm text-slate-400 italic">
                                    Gagal memuat atau menyusun soal kuis
                                    latihan.
                                </p>
                            </div>
                        )}
                    ...
                    <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs">
                        ...
                        {loadingLms ? (
                            ...
                            <div className="text-center py-8 text-slate-400 font-medium italic text-sm">
                                Belum ada tugas yang diberikan untuk Modul atau
                                Kanji ini.
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {lmsAssignments.map((assign) => {
                                    ...
                                    const currentAssignmentComments =
                                        lmsComments.filter(
                                            (comm) =>
                                                comm.assignmentId === assign.id,
                                        );
                                        ...
                                        >
                                            ...
                                            <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs">
                                                ...
                                                <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1 sidebar-scroll">
                                                    {currentAssignmentComments.length ===
                                                    0 ? (
                                                        <div className="text-center py-4 text-slate-400 font-medium italic text-xs">
                                                            Belum ada diskusi
                                                            untuk tugas ini.
                                                            Mulai kirim komentar
                                                            pertama Anda!
                                                        </div>
                                                    ) : (
                                                        currentAssignmentComments.map(
                                                            (comm) => {
                                                                const isOwner =
                                                                    currentUser &&
                                                                    comm.userId ===
