...
async function fixAllScrambledWords() {
  console.log('Fixing scrambled words pool for Kanji 問, 験, and 試...');

  // ==========================================
  // 1. KANJI 問 (ID: 3214)
  // ==========================================
  const kanjiMon = await prisma.kanji.findFirst({
    where: { character: '問' }
  });
  ...
  if (kanjiMon) {
    ...
    const monUnscrambleQuizzes = [
      {
        kanjiId: kanjiMon.id,
        type: 'unscramble',
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: あったら、・先生・が・問題/質問・に・もし・聞いて下さい
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 質問 ・ 先生 ・ が ・ あるん ・ に ・ です
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 設問 ・ が ・ に ・ 書いて ・ あります
        words: JSON.stringify(['設問', 'が', 'に', '書いて', 'あります']),
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 反問した ・ ん ・ 先生 ・ です ・ に ・ 彼は
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 予定 ・ を ・ 先生の家 ・ です ・ 来週 ・ 訪問する
    ...
    for (const q of monUnscrambleQuizzes) {
      await prisma.quiz.create({ data: q });
    }
    console.log(`Created 5 scrambled unscramble quizzes for Kanji 問.`);
  }
  ...
  if (kanjiKen) {
    ...
    const kenUnscrambleQuizzes = [
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 明日 ・ 試験 ・ が ・ から ・ 勉強します ・ ある
        words: JSON.stringify(['明日', '試験', 'が', 'から', '勉強します', 'ある']),
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 経験 ・ 働いた ・ が ・ あります ・ 日本で
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 文化 ・ を ・ 体験しました ・ 京都 ・ で
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 実験 ・ を ・ 料理室 ・ しました ・ で
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut menjadi kalimat yang tepat.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 受検しようと ・ 日本語の ・ 思っています ・ 試験を
    ...
    console.log(`Created 5 scrambled unscramble quizzes for Kanji 験.`);
  }
  ...
  if (kanjiShi) {
    ...
    const shiUnscrambleQuizzes = [
      {
        ...
        options: JSON.stringify([]),
        // Scrambled pool from prompt: ありました ・ 試合 ・ きのう ・ が
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut ini menjadi kalimat yang benar.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 試食 ・ しました ・ 新しいパンを ・ スーパーで
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut ini menjadi kalimat yang benar.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: 試飲 ・ けさ ・ ジャム ・ しました ・ 田中さん ・ を ・ は
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut ini menjadi kalimat yang benar.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: します ・ 前に ・ 試乗 ・ 買う ・ 新しい車を ・ まず
      ...
      },
      {
        ...
        question: 'Susunlah kata-kata berikut ini menjadi kalimat yang benar.',
        options: JSON.stringify([]),
        // Scrambled pool from prompt: あったので ・ 昨日 ・ 休みました ・ 試合が ・ 大学を
    ...
    console.log(`Created 5 scrambled unscramble quizzes for Kanji 試.`);
  }

  await prisma.$disconnect();
}

fixAllScrambledWords().catch(err => {
  console.error(err);
  prisma.$disconnect();
  process.exit(1);
});
